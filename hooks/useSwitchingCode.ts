...
export default function useSwitchingCode(
  code: string,
  ms = 20,
  progress = 1,
  fill = true,
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const maxLines = fill
        ? 10
        : Math.max(prevLines.length, currentLines.length);
      ...
      const remainingLines = prevLines.map((line, index) => {
        ...
        return (
          (currentLines[index]?.slice(
            0,
            Math.floor(currentLines[index].length * (i / 30)),
          ) ?? "") +
      ...
      setValue(
        (fill
          ? remainingLines
          : remainingLines.filter((line, index, arr) => {
